...
def deep_copy_Node(oNode):
    ...
    nNode = Node([row[:] for row in oNode.current_state])
    return nNode
...
class Puzzle(object):
    ...
    # def ids(self, cNode, pNode, visited, currDepth, maxDepth, goalState, originalState, pred, dirs):
    def ids(self, cNode, pNode, visited, currDepth, maxDepth, goalState, originalState):
        # print("here?")
        # print(cNode[0].current_state, originalState)
        current_parsed_state = tuple(map(tuple, cNode[0].current_state))
        previous_parsed_state = tuple(map(tuple, pNode[0].current_state))

        if current_parsed_state != originalState:
            prev[current_parsed_state] = previous_parsed_state
            dirs[current_parsed_state] = cNode[1]
            # print(prev, dirs)

        visited.add(current_parsed_state)
        if current_parsed_state == goalState:
            print("Found end")
            # print(prev, dirs)
            return True
        ...
        neighbours = self.generate_neighbours(cNode[0])
        for nNode in neighbours:
            # print ("nn: ", nNode)
            parsed_state = tuple(map(tuple, nNode[0].current_state))
            if parsed_state not in visited:
                if self.ids(nNode, cNode, visited, currDepth + 1, maxDepth, goalState, originalState):
                    return True
    ...
    def solve(self):
        ...
        # dirs = {}
        # dirs[tuple(map(tuple, self.init_state))] = -1
        #endregion 
        ...
        goalState = tuple(map(tuple, self.goal_state))
        maxDepth = 60
        for i in range(1, maxDepth + 1):
            print("Depth: ", i)
            if self.ids(mNode, mNode, visited, 0, i, goalState, initState):
                # print ("Got end: ", prev)
                break

            # print("Clearing everything")
            # print("Before ", prev, dirs, visited)
            prev.clear()
            dirs.clear()
            visited.clear()
            # print("After ", prev, dirs, visited)
