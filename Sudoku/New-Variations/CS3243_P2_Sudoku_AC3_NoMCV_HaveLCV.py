...
def backtrack(state, totalBacktracks = 0, depth = 0, maxDepth = 0, count = 0):
    count += 1
    depth += 1
    if maxDepth < depth:
        maxDepth = depth
    ...
    for val in sorted_domain: 
        ...
        if count > 0:
            ...
            # if forward_checking(new_state):
                result, totalBacktracks, depth, maxDepth = backtrack(new_state, totalBacktracks, depth, maxDepth, count)
                if result:
                    #print str(count)
                    return result, totalBacktracks, depth, maxDepth
        else:
